...
class TestHaVLAN(TestBasic):
    ...
    @log_snapshot_on_error
    @test(groups=["thread_3"], depends_on=[deploy_ha_vlan])
...
class TestHaFlat(TestBasic):
    ...
    @log_snapshot_on_error
    @test(groups=["thread_4"], depends_on=[deploy_ha_flat])
...
class TestHaFlatAddCompute(TestBasic):
    ...
    @log_snapshot_on_error
    @test(groups=["thread_4"], depends_on=[TestHaVLAN.deploy_ha_flat])
    ...
    @log_snapshot_on_error
    @test(groups=["thread_4"], depends_on=[ha_flat_add_compute])
